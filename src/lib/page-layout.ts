...
/** Расстояние между листами в режиме «Разметка страницы» (серая прослойка, ближе к Word). */
...
export const PAGE_CHROME = {
  /** Фон вокруг листов (рабочая область), как в Word «Разметка страницы». */
  workspaceBackground: "#e8e8e8",
  /**
   * Край листа + мягкая тень (Word-подобно, без тяжёлой обводки).
   */
  sheetShadow:

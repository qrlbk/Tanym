...
## Локальные API-ключи в desktop

В Tauri-сборке ключи OpenAI / Anthropic / Google можно добавить через меню:
`Файл -> Настройки AI-ключей`.

- Хранение: системный keychain ОС (локально на устройстве пользователя).
- Ключи не пишутся в файлы проекта, `localStorage` или IndexedDB.
- Удаление ключей доступно там же, в настройках AI-ключей.
- В desktop-режиме облачные AI-маршруты читают ключи только из keychain (без fallback на `.env`).
